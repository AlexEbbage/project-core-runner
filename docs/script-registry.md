...
| `TunnelWallGeneratorV2.cs` | `Assets/CoreRacer/Runtime/Gameplay/Environment` | Gameplay | Generates and contrast-tints the selected polygon tunnel mesh, then recenters it ahead of the player without modifying the shared material or authored hierarchy | `RunController`, player transform, route side count, tunnel renderer/material | F1, F12 | Active |
| `RunController.cs` | `Assets/CoreRacer/Runtime/Gameplay/Run` | Gameplay | Orchestrates validated clean-scene run startup, selected-route configuration, lifecycle transitions, and application/reset of run-only booster modifiers | Run references/config, roadmap, profile, booster catalog, gameplay systems | F1, F4, F12, F20 | Active |
...
| `TutorialDirector.cs` | `Assets/CoreRacer/Runtime/FTUE` | UI | Thin scene coordinator that reacts to tutorial step changes and routes deterministic pickup assists plus hub prompts | Tutorial service, run/pickup/obstacle controllers, menu router, overlay | F21 | Active |
