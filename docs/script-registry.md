...
| `MainMenuUI.cs` | `Assets/Scripts/UI/MainMenu` | UI | Play-page presentation, best score, lock-aware level selection, level-up feedback, side-entry routing for tasks/daily login, and remove-ads entry points | GameManager, level selection data, monetisation UI, progression surfaces | F7, F8, F10, F11, F12, F14, F18 | Active |
...
| `AchievementsPageController.cs` | `Assets/Scripts/UI/MainMenu` | UI | Builds the first achievements shell on the mapped challenges page, evaluates progress, and handles tier reward claiming | PlayerProfile, achievements config, localization, hub refresh | F19, F8 | Active |
| `ProgressionTasksController.cs` | `Assets/Scripts/UI/MainMenu/Progression` | UI | Populates daily/weekly/monthly task UI, resolves cadence state, and handles task plus milestone reward claiming | PlayerProfile, progression task config, task/reward views, hub refresh | F11, F8 | Active |
...
| `DailyLoginRewards.cs` | `Assets/Scripts/Meta` | Meta | Daily login reward config and manager for claim cadence and reward granting | PlayerProfile, daily reward config | F10, F8 | Active |
