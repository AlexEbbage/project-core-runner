...
| F12-T3 | F12 | Replace the single scene-authored route with a five-entry polygon roadmap config and challenge-rich cards | Codex/User | Implemented (awaiting review) | Uses `LevelRoadmapConfig`, keeps index-based selection persistence, and varies tunnel shape through `sides` only |
...
| T4 | Level Roadmap | Complete environment unlock and level-select product definition | Codex/User | Implemented (awaiting review) | The level roadmap is now authored as five polygon routes with profile-level gates and display-only challenge goals |
