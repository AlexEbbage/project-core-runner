...
| F7-T1 | F7 | Remap the existing main-menu pages into the target hub destinations and make `Level Select` the default landing page | Codex/User | In progress | Covers bottom-nav routing for shop, ship, lab, levels, and achievements |
| F7-T2 | F7 | Add side-entry hub actions and badge visibility for daily login, special offers, tasks, and notifications | Codex/User | In progress | Keep badge state lightweight and sourced from existing managers/controllers |
| F8-T1 | F8 | Keep the hub top bar in sync with profile level, XP, and currency state | Codex/User | In progress | Currency taps should continue routing into the shop currency surface |
