...
| 6 | Progression | Ship Customisation | F9, F17 | Equipable ship parts with persisted loadout and unlock ownership | Ship preview, equip slots, tabbed grids for item categories | Selection should update the preview immediately and support quick comparison | Unlock/equip cheats, preview swapper, missing-art warnings | In progress | The next active slice is to turn the current hangar shell into a clearer player-facing customisation flow, reusing the newly authored shop ids and persisted ownership state. |
...
| F17 | Meta | Expanded Content Catalog | In progress | Shop starter content is now authored and the next active slice is expanding that content into a clearer ship customisation flow built on the same ownership ids |
