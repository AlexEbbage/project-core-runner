...
| F5 | Core Gameplay | Powerup Gameplay Loop | In progress | Runtime powerups are present and the current slice is converging the roster, HUD support, and upgrade fallback behavior |
...
| F13 | Services | Ads and Reward Monetisation | In progress | Continue ads and reward-doubling exist, and the mid-run rewarded-offer loop is now being integrated as the next production slice |
